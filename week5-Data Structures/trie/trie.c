...
bool check(char *word)
{
    ...
    {
        // convert current character to index in the children array
        int index = tolower(word[i]) - 'a';

        // if the current node has no child for the character, the word is not in the trie
        if (cursor->children[index] == NULL)
        {
            return false;
        }
    ...
    // if we reach the end of the word and the last node is marked as a word, the word is in the trie
